...
public class ParkingSpace implements Runnable {
  ...
  private void registersWithMonitor() throws IOException {
    ...
    try {
      ...
      reportError("registersWithMonitor: malformed url: " + this.monitorAddr.toString() + ": "
          + e.getMessage());
    }
    // System.out.println("url stuff: " + url.getHost());
  ...
  public void http() {
    try {
      ...
      httpServer.bind(new InetSocketAddress("localhost", this.httpPort),
          SystemConfig.MAXIMUM_HTTP_CONNECTIONS);
    } catch (IOException e) {
      reportError("Unable to create the http service for parking space: " + e.getMessage());
      System.exit(1);
    }
    httpServer.createContext(SystemConfig.PARKING_SPACE_CAR_ENTERING_PATH,
        new CarEnteringHttpHandler(this));
    httpServer.createContext(SystemConfig.PARKING_SPACE_PEER_ADDRESS_CHANGE_PATH,
        this.gateAddressBook);
    httpServer.start();
  }
