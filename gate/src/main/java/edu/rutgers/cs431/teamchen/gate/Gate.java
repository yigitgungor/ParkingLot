...
public class Gate implements Runnable {
  ...
  protected final int gateTcpPort;
  ...
  public void registerThenInit() {
    ...
    try {
      req = new GateRegisterRequest(InetAddress.getLocalHost().getHostName(), this.gateTcpPort,
          this.gateHttpPort);
    ...
    try {
      ...
      reportError(
          "invalid Parking Space's HTTP URL: " + resp.parkingSpaceHttpUrl + ": " + e.getMessage());
    }
  ...
  public void http() {
    try {
      ...
      httpServer.bind(new InetSocketAddress("localhost", this.gateHttpPort),
    ...
    while (true) {
      ...
      try {
        ...
        if (currentTime > next.car.getDepartureTimestamp()) {
          ...
          lastTimeProcessedCar = this.clock.getTime();
          carsProcessedCount++;
          continue;
        }
  ...
  private void sendCarToParkingSpace(CarWithToken cwt) {
    ...
    try {
      ...
      reportError("unable to send car with token " + cwt.token + " to the parking space: "
          + e.getMessage());
  ...
  public void run() {
    ...
    log("Listening to Traffic Generator at "
        + this.carsAcceptor.getLocalSocketAddress().toString());
